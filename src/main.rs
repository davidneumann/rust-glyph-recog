...
fn main() -> io::Result<()> {
    ...
    let ray2 = &get_rays(&(input.to_owned() + "70.dat"));
    println!("108/70");
    print_rays(ray2);
    ...
    let ray_l = &get_rays(&(input2.to_owned() + "73/0.dat"));
    println!("73/0");
    print_rays(ray_l);
        ...
        .map(|x| 
             {
                 ...
                 let files = fs::read_dir(input2.to_owned() + &dir_name).unwrap()
                     ...
                     .map(|x| x.unwrap());
                 let mut max_width = 0;
                 let mut min_input = "".to_string();
                 for file in files {
                     let file_name = file.path().file_name().unwrap().to_str().unwrap().to_owned();
                     let width = get_width_from_dat(&(input2.to_owned() + &dir_name + "/" + &file_name));
                     if width > max_width {
                         max_width = width;
                         min_input = file_name.to_string();
                     }
                 }
                 println!("Using {} for {}", min_input, dir_name);
                 (c, get_rays(&(input2.to_owned() + &dir_name + "/" + &min_input)))
             })
    .collect();
...
fn get_width_from_dat(input: &str) -> i32 {
...
fn get_rays(input: &str) -> GlyphRays {
    ...
    loop {
        ...
        if buffer[0] != 0 {
            ...
            if x <  width  / 2 { rays.m2l[(y as usize)] = cmp::min(x, rays.m2l[(y as usize)]); }
            if x >=  width  / 2 { rays.m2r[(y as usize)] = cmp::min(width - x - 1, rays.m2r[(y as usize)]); }
            if y <  height / 2 { rays.m2t[(x as usize)] = cmp::min(y, rays.m2t[(x as usize)]); }
            if y >=  height / 2 { rays.m2b[(x as usize)] = cmp::min(height - y - 1, rays.m2b[(x as usize)]); }
            // print!("X");
        }
        //else { print! (" ");}
...
fn get_ray_delta(r1: &GlyphRays, r2:&GlyphRays) -> i32 {
    ...
    for y in 0..=max_height {
        ...
        delta += get_vec_delta(&r1.r2l, &r2.r2l, y as usize, max_width);
        delta += get_vec_delta(&r1.m2r, &r2.m2r, y as usize, max_width);
        delta += get_vec_delta(&r1.m2l, &r2.m2l, y as usize, max_width);
    }

    //Vertical vecs
    for x in 0..=max_width  {
        delta += get_vec_delta(&r1.t2b, &r2.t2b, x as usize, max_height);
        delta += get_vec_delta(&r1.b2t, &r2.b2t, x as usize, max_height);
        delta += get_vec_delta(&r1.m2b, &r2.m2b, x as usize, max_height);
        delta += get_vec_delta(&r1.m2t, &r2.m2t, x as usize, max_height);
    }
...
fn get_vec_delta(l:&Vec<i32>, r:&Vec<i32>, index:usize, max_value:i32) -> i32 {
    ...
    //    else { r[index] };
    let left =
        if index < l.len() + 2 { l[cmp::min(index, l.len() - 1)] } else { max_value };
    let right = 
        if index < r.len() + 2 { r[cmp::min(index, r.len() - 1)] } else { max_value };
    (right - left).abs()
}
...
fn print_rays(rays: &GlyphRays) -> () {
    ...
    for y in 0..rays.height {
        for x in 0..rays.width {
            ...
            // else if rays.width / 2 - rays.m2r[(y as usize)] == x { print!("X"); }
            // else if rays.height / 2 - rays.m2t[(x as usize)] == y { print!("X"); }
            // else if rays.height / 2 - rays.m2b[(x as usize)] == y { print!("X"); }
            else { print!(" "); }
        }
        println!();
    }
